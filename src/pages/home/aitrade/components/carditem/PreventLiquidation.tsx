...
export default ({ toolt, icon, title, field, user, api }: any) => {
  ...
  let get_prevent_liquidation_amount = 1000;
  ...
  const MyLiquidation = () => {
    return (
      <AlertDialogContent>
        <AlertDialogHeader fontSize="lg" fontWeight="bold">
          {intl.formatMessage({ id: "trade.config.item6.title" })}
        </AlertDialogHeader>
        <Flex fontSize="1.2rem" textAlign="center" pl={6} alignItems="center">
          ...
          <AlertDialogBody>
            <Select
              ...
              textAlign="center"
              size="sm"
              // defaultValue={get_prevent_liquidation_amount}
              defaultValue={get_prevent_liquidation_amount}
              onChange={(e: any) => {
                get_prevent_liquidation_amount = parseInt(e.target.value);
                console.log(e.target.value, "eeee");
              }}
            >
              <option value="1000">1000USDC</option>
              <option value="5000">5000USDC</option>
              <option value="10000">10000USDC</option>
              <option value="20000">20000USDC</option>
              <option value="50000">50000USDC</option>
              <option value="100000">100000USDC</option>
            </Select>
          </AlertDialogBody>
        </Flex>
        <AlertDialogFooter>
          ...
          <Button
            ...
            onClick={() => {
              doRequest({
                key: field,
                value: value === "1" ? "0" : "1",
                prevent_liquidation_amount: get_prevent_liquidation_amount,
              });
            }}
            ml={3}
          >
